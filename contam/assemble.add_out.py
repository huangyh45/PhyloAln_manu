...
class Assembler:
	...
	def remove_out_hit(self, weight=1):
		to_remove = []
		outfile = open(os.path.join('stat_info', self.name + '_' + self.species + '.out_clean_info.tsv'), 'w')
		for i in range(len(self.hits)):
			hit = self.hits[i]
			# test: not to remove the same species
			# if hit.hit_id.startswith(self.species + '_'):
			#	continue
			score = 0
			outgroup_score = 0
			...
			if score < outgroup_score * weight:
				to_remove.append(i)
				seq = ''
				for pos in range(hit.query_start, hit.query_end+1):
					if hit.moltype.startswith('dna'):
						seq += hit.seq_comp[pos]
					else:
						for k in [1, 2, 3]:
							seq += hit.seq_comp[pos]['codon'][k-1]
				outfile.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(seq, hit.query_start, hit.query_end, outgroup_score, score, ','.join(hit.reads)))
		outfile.close()
		temp_hits = []
		for i in reversed(to_remove):
			if self.hits[i].hit_id.startswith(self.species + '_'):
				temp_hits.append(self.hits[i])
		self.hits = temp_hits
		...
		if final_seq.startswith('consensus'):
			...
			for hit in assembler.hits:
				...
				for read in hit.reads:
					if read not in assembler.hits[0].reads:
						assembler.hits[0].reads.append(read)
			start_pos = 0
			end_pos = 0
...
def cross_decontam(assemblers, total_reads, min_overlap=30, min_pident=98, min_expression=0.2, min_fold=2):
	...
	outfile = open(os.path.join('stat_info', list(assemblers.values())[0].name + '.cross_clean_info.tsv'), 'w')
				...
				for j in range(len(assembler2.hits)):
					...
					if overlap_len:
						...
						if exp_fold > min_fold and assembler1.hits[i].pseudoRPKM >= min_expression and j not in to_remove[sp2]:
							...
							outfile.write("{}\t{}\t{}\n".format(sp1, sp2, ','.join(assembler2.hits[j].reads)))
							...
							outfile.write("{}\t{}\t{}\n".format(sp2, sp1, ','.join(assembler1.hits[i].reads)))
	statout.close()
	outfile.close()
	outfile = open(os.path.join('stat_info', list(assemblers.values())[0].name + '.clean_info.tsv'), 'w')
	for sp, remove_indexes in to_remove.items():
		...
		for hit in assemblers[sp].hits:
			outfile.write("{}\t{}\n".format(sp, ','.join(hit.reads)))
	outfile.close()
	return assemblers
...
def cross_and_output(group_name, assemblers, sps, total_reads, moltype='dna', gencode=1, no_assemble=False, no_cross_species=False, min_overlap=30, min_pident=98, min_exp=0.2, min_fold=2, unknow='unknow', final_seq='consensus', no_ref=False, sep='.'):
	...
	if not no_cross_species and len(sps) > 1 and no_assemble and final_seq.startswith('consensus'):
		...
		outfile = open(os.path.join('stat_info', group_name + '.cross_clean_info.tsv'), 'w')
		for sp1 in sps:
			for sp2 in sps:
				if sp1 == sp2:
					continue
				...
				if exp_fold > min_fold and pseudoRPKM1 > min_exp:
					...
					if not assemblers[sp2].moltype.startswith('dna'):
						protseqs.pop(seqid2)
					outfile.write("{}\t{}\t{}\n".format(sp1, sp2, ','.join([','.join(hit.reads) for hit in assemblers[sp2].hits])))
				elif exp_fold < 1 / min_fold and pseudoRPKM2 > min_exp:
					seqs.pop(seqid1)
					if not assemblers[sp1].moltype.startswith('dna'):
						protseqs.pop(seqid1)
					outfile.write("{}\t{}\t{}\n".format(sp2, sp1, ','.join([','.join(hit.reads) for hit in assemblers[sp1].hits])))
		crossout.close()
		outfile.close()

	outfile = open(os.path.join('stat_info', group_name + '.clean_info.tsv'), 'w')
	for seqid in seqs.keys():
		sp = seqid.split(sep)[0]
		if assemblers.get(sp) is not None:
			outfile.write("{}\t{}\n".format(sp, ','.join([','.join(hit.reads) for hit in assemblers[sp].hits])))
	outfile.close()
