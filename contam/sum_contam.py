...
outfile.close()
outfile = open('sum_contam.PR.tsv', 'w')
outfile.write("species\ttype\torthogroup\tmapped reads\tTP\tFP\tFN\tTN\taccuracy\tprecision\trecall\tF-Measure\n")
for sp in sps:
	for filename in os.listdir('/public/lihaosen/PhyloAln/PhyloAln_contam/aa_out'):
		og = filename.replace('.fa', '')
		reads = {'clean': {}, 'outgroup': {}, 'cross': {}}
		for line in open('/public/lihaosen/PhyloAln/PhyloAln_contam/stat_info/' + og + '.clean_info.tsv'):
			arr = line.rstrip("\n").split("\t")
			if arr[0] == sp:
		...
		for line in open('/public/lihaosen/PhyloAln/PhyloAln_contam/stat_info/' + og + '.cross_clean_info.tsv'):
			arr = line.rstrip("\n").split("\t")
			if arr[1] == sp:
		...
		for line in open('/public/lihaosen/PhyloAln/PhyloAln_contam/stat_info/' + og + '_' + sp + '.out_clean_info.tsv'):
			arr = line.rstrip("\n").split("\t")
